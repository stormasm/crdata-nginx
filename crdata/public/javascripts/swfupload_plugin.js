...
var FlashUploader = Class.create({
    ...
    param_pairs = this.swfUploadBlock.down('.post_params').innerHTML.split(', ')
    param_pairs.each(function(pair) {
      param = pair.split(':');
      this.includeParams.set(param[0], param[1]);
    }, this)
    ...
    this.includeParams.each(function(pair) {
      this.swfu.addPostParam(pair.key, $(pair.value).value )
    }, this)
	},

module GroupsHelper
  ...
  def get_group_actions(record)
    ...
    if current_user
      ...
      html += link_to('Edit', edit_group_path(record))  if group_user and (group_user.status == 'approved') and ['Admin', 'Owner'].include?(group_user.role.name) 
      html += link_to('Destroy', record, :confirm => 'Are you sure you want to delete this group?', :method => :delete)  if group_user and (group_user.status == 'approved') and (group_user.role.name == 'Owner')
    end
    html
  end

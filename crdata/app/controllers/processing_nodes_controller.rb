class ProcessingNodesController < ApplicationController
  before_filter :require_user,              :except => [:index, :show, :register, :unregister, :manage_donation, :do_manage_donation]
  before_filter :require_jobs_queue_admin,  :only =>   [:edit, :update, :destroy]
  before_filter :require_jobs_queue_access, :only =>   [:show]
  before_filter :require_site_admin,        :only =>   [:manage_donation, :do_manage_donation]
  ...
  def create
    ...
    respond_to do |format|
      if @processing_node.save_node(current_user, params)
        ...
        @processing_node.save_aws_key(current_user, params[:aws_credentials]) if params[:save_aws_keys] and (params[:node_type] == 'automatic') and (current_user.is_site_admin? or current_user.jobs_queues_admin.include?(@processing_node.jobs_queue))
  ...
  def destroy
    respond_to do |format|
      ...
      format.html { redirect_to(processing_nodes_url) }
      format.xml  { head :ok }
      else
        flash[:notice] = result
        format.html { redirect_to(processing_nodes_path) }

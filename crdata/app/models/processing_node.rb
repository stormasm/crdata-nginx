class ProcessingNode < ActiveRecord::Base
  ...
  def save_node(user, parameters)
    begin
      if parameters[:node_type] == 'automatic'
        ec2 = RightAws::Ec2.new(parameters[:aws_credentials][:access_key_id], parameters[:aws_credentials][:secret_access_key])
        key = 'crdata_key'
        ...
        security_group = AwsKey.is_crdata_key?(parameters[:aws_credentials][:access_key_id]) ? EC2_SECURITY_GROUP : 'default'
        ec2_instances = ec2.run_instances(parameters[:ec2_instance_ami], 1, 1, [security_group], key, "url='" + CRDATA_HOST + "', uid='" + self.uuid + "'")
  ...
  def save_aws_key(user, aws_credentials)
    self.aws_key = AwsKey.create(:access_key_id => aws_credentials[:access_key_id], :secret_access_key => aws_credentials[:secret_access_key], :user_id => user.id)
    self.save! 
  end

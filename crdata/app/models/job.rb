class Job < ActiveRecord::Base
  ...
  validates_inclusion_of :status, :in => %w( new submitted running done cancelled ), :message => 'Valid values are: new, submitted, running, cancelled and done'
  ...
  def uploadurls(upload_type, files)
    ...
    raise 'Invalid upload type' if !upload_type || ![:logs, :results].include?(upload_type.to_sym)
...
EOS
  ...
  def check_state_transition    
    ...
    $ALLOWED_STATUSES ||= { nil => ['new'], 'new' => ['submitted', 'cancelled'], 'submitted' => ['running', 'cancelled', 'new'], 
  ...
  def self.get_conditions(user, statuses)
    ...
    conditions += " AND status IN ('#{selected_statuses.join("', '")}')" if !selected_statuses.blank?
